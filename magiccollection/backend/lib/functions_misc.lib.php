 ...
 */
function validateDate($date, ?string $format = 'Y-m-d') {
	$d = DateTime::createFromFormat($format, $date);
	return $d && $d->format($format) == $date;
}

/**
 * Generate a random string of specified length. If no length specified, returns a random alphanumerical character
 *
 * @param int $l : length of string
 * @return string
 */
function randomString(?int $l = 1) {
